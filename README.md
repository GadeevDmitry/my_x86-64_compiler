...
2. Все функции должны иметь возвращаемое значение во внешней области видимости.
3. Доступны арифметические операции `+, -, *, /`.
4. Доступны логические операции `>, <, >=, <=, &&, ||, !`.
5. Доступна бинарная операция `^`, которая извлекает квадратный корень из первого (левого) операнда, а второй (правый) операнд игнорирует.
...
8. Грамматика ManCityLang (примеры в папке examples):
...
| IR_CMD                                                        | Описание                                                                                   |
|---------------------------------------------------------------|--------------------------------------------------------------------------------------------|
| ADD, SUB, MUL, DIV, POW, OR, AND, ARE_EQ, NOT_EQ, LESS, MORE, LESS_EQ, MORE_EQ, NOT | Извлечение операндов из стека -> операция -> добавление результата в стек |
